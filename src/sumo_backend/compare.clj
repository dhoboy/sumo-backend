...
(defn losses-to-rank
  ...
  [{:keys [rikishi comparison rank-str year month day] :or {comparison =}}] ; "endo" >= "ozeki"
    (rikishi-comparison ; all losses against ozeki or higher
      ...
      #(let [rank-str-value (utils/get-rank-value rank-str)
             opponent-rank-value (utils/get-opponent-rank-value-in-bout rikishi %)]
...
(defn losses-to-lower-rank
  ...
  [{:keys [rikishi comparison delta year month day] :or {comparison <= delta ##Inf}}]
    (rikishi-comparison ; :comparison can be >, >=, =, <, <=
      rikishi            
      "lose"
      #(let [rikishi-rank-value (utils/get-rank-value-in-bout rikishi %)
             opponent-rank-value (utils/get-opponent-rank-value-in-bout rikishi %)]
